...
# TODO: перестроить архитетуру в соответствии с паттерном "Стретегия"
class InputValidator(Cleaner):
    """
    Класс, позволяющий принять файл и конвертировать его в <padndas.DataFrame>
    """
    ...
    @Logger.info_log()
    def allowed_file(self) -> bool: #filename
        """
    ...
    def convert_file_to_df(self) -> pd.DataFrame:
        ...
        if self.allowed_file():
            if not self._is_password_protected():
                ...
                # TODO: как собрать dataframe из excel файла, убрав дублирующиеся столбцы?
